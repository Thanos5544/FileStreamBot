...
def create_gradient(size, direction="left"):
    ...
    if direction == "left":
        for x in range(w):
            a = max(0, min(255, int(255 * (1 - x / (w * 0.6)))))
            d.line([(x, 0), (x, h)], fill=(0, 0, 0, a))
    else:
        for y in range(h):
            a = max(0, min(255, int(255 * (y / h))))
...
async def create_poster(image_url, movie_data, color_name=None, branding=None, channel=None):
    ...
    if color_name and color_name in COLORS:
        c = COLORS[color_name]
        overlay = Image.new("RGBA", target, (c[0], c[1], c[2], 90))
        img = Image.alpha_composite(img, overlay)
    
    img = Image.alpha_composite(img, create_gradient(target, "left"))
    img = Image.alpha_composite(img, create_gradient(target, "bottom"))
    ...
    if channel:
        fc = get_font(22, bold=True)
        draw.rectangle([(30, 30), (60, 55)], fill=(255, 255, 255, 40))
        draw.text((70, 32), channel, font=fc, fill=(255, 255, 255, 255))
    
    if director:
        ff = get_font(16, bold=True)
        dc = COLORS[color_name] if color_name and color_name in COLORS else (46, 204, 113)
        draw.text((30, 200), "FILMED BY", font=ff, fill=(255, 255, 255, 200))
        b = draw.textbbox((0, 0), "FILMED BY", font=ff)
        fw = b[2] - b[0]
        draw.text((30 + fw + 10, 200), director[:30], font=ff, fill=dc)
    ...
    draw.text((30, 240), tu, font=ft, fill=(255, 255, 255, 255))
    
    lc = COLORS[color_name] if color_name and color_name in COLORS else (46, 204, 113)
    tb = draw.textbbox((30, 240), tu, font=ft)
    lw = min(tb[2] - tb[0], 200)
    ly = tb[3] + 10
    draw.rectangle([(30, ly), (30 + lw, ly + 4)], fill=lc)
    
    fi = get_font(20, bold=True)
    it = year
    if gstr:
        it += f"  •  {gstr}"
    if duration:
    ...
    if overview:
        ...
        sy = iy + 40
        for line in lines:
            draw.text((30, sy), line, font=fs, fill=(200, 200, 200, 220))
            sy += 22
    ...
    btw = b2[2] - b2[0]
    btx = 30 + (bw - btw) // 2
    bty = by + (bh - (b2[3] - b2[1])) // 2 - 5
    draw.text((btx, bty), bt, font=fb, fill=(255, 255, 255, 255))
    
    ix = 30 + bw + 15
    iw = 130
    
    draw.rounded_rectangle([(ix, by), (ix + iw, by + bh)], radius=8, fill=(20, 20, 20, 220), outline=(255, 200, 0, 255), width=2)
    
    fst = get_font(24, bold=True)
    draw.text((ix + 15, by + 12), "✦", font=fst, fill=(255, 200, 0, 255))
    
    fr = get_font(18, bold=True)
    rt = f"{rating:.1f} IMDb" if rating else "N/A IMDb"
    draw.text((ix + 45, by + 18), rt, font=fr, fill=(255, 255, 255, 255))
    
    if branding:
        fw2 = get_font(20, bold=True)
        wb = draw.textbbox((0, 0), branding, font=fw2)
        ww = wb[2] - wb[0]
        wx = (target[0] - ww) // 2
        wy = target[1] - 40
        draw.text((wx + 2, wy + 2), branding, font=fw2, fill=(0, 0, 0, 200))
